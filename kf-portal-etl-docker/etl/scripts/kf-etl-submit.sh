...
/kf-etl/spark/bin/spark-submit --master local[*] --class io.kf.etl.ETLMain --driver-java-options "${KF_ETL_CONFIG}" --conf "spark.executor.extraJavaOptions=${KF_ETL_CONFIG}" ${ETL_JAR_URL} -study_id  ${KF_STUDY_ID} -release_id ${KF_RELEASE_ID}
