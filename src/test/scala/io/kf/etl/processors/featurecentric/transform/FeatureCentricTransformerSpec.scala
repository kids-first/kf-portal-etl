...
import io.kf.etl.processors.{Data, Data2}
...
class FeatureCentricTransformerSpec extends FlatSpec with Matchers with WithSparkSession {
  ...
  "fileCentric" should "return the proper Sequence of FileCentric_ES" in {
    ...
    val result = FeatureCentricTransformer.fileCentric(entityDataSet, entityDataSet.participants.map(EntityConverter.EParticipantToParticipantES))
