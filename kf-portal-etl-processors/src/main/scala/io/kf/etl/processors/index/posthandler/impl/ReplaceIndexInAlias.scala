  ...
  override def post(): Unit = {
    ...
    val remove_request = new IndicesAliasesRequest
      ...
      WrapAsScala.asScalaIterator(
        Context.esClient.admin().indices().getAliases((new GetAliasesRequest).aliases(alias)).actionGet().getAliases.iterator()
      ).foreach(tuple => {
    ...
    val remove_response = Context.esClient.admin().indices().aliases(remove_request).actionGet()

    remove_response.isAcknowledged match {
      case true => {
        val add_req = new IndicesAliasesRequest

        Context.esClient.admin().indices().aliases(add_req.addAliasAction(AliasActions.add().index(s"${alias}_${context.config.releaseTag.releaseTag()}").alias(alias))).actionGet()
      }
      case false => {
