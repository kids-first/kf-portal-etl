...
import io.kf.etl.processors.common.ProcessorCommonDefinitions.{DataServiceEntityNames, DatasetsFromDBTables, EntityDataSet}
...
//    data.participantAlis.write.parquet(s"${context.getJobDataPath()}/${Participant_Alias.toString}")
  ...
  def sink(data: EntityDataSet):Unit = {
    checkSinkDirectory(new URL(context.getJobDataPath()))
